package cm.inv.com.crawler.common.persistence;


import java.util.List;


/**
 * DAO支持类实现
 * @author zmwy
 * @version 2014-05-16
 * @param <T>
 */

public interface CrudDao<T> extends BaseDao {

	/**
	 * 获取单条数据
	 * @param id
	 * @return
	 */
	public T get(String id);
	public Integer getBuildingCount(String communityid);
	public T getByMobile(String mobile);
	public List<T> getHUserByMobile(T houseUser);
	public T getUserByMobile(String mobile);
	public T getAdCheck(String adId);
	
	/**
	 * 根据广告位id获取广告
	 */
	public T getAdByPositionId(String positionId);
	
	/**
	 * 获取单条数据
	 * @param entity
	 * @return
	 */
	public T get(T entity);
	/**
	 * 获取回访信息
	 * @param
	 * @return
	 */
	public T getVisit(String complaintId);
	
	/**
	 * 查询数据列表，如果需要分页，请设置分页对象，如：entity.setPage(new Page<T>());
	 * @param entity
	 * @return
	 */
	public List<T> findList(T entity);
	
	public List<T> findAreaList(T entity);
	public void batchupdateCategoryStatus(List<T> list);
	/**
	 * 查询所有数据列表
	 * @param entity
	 * @return
	 */
	public List<T> findAllList(T entity);
	/**
	 * 查询数据列表name
	 * @param
	 * @return
	 */
	public List<T> getCommunityname();
	/**
	 * 查询所有数据列表
	 * @see public List<T> findAllList(T entity)
	 * @return
	 */
	@Deprecated
	public List<T> findAllList();
	
	/**
	 * 插入数据
	 * @param entity
	 * @return
	 */
	public int insert(T entity);
	
	/**
	 * 更新数据
	 * @param entity
	 * @return
	 */
	public int update(T entity);
	
	/**
	 * 将小区写到offfice中
	 * @param entity
	 */
	public void communitySaveOffice(T entity);
	
	/**
	 * 修改写到office中的小区信息
	 * @param entity
	 */
	public void communityUpdateOffice(T entity);

	
	/**
	 * 删除数据（一般为逻辑删除，更新del_flag字段为1）
	 * @param id
	 * @see public int delete(T entity)
	 * @return
	 */
	@Deprecated
	public int delete(String id);
	
	/**
	 * 删除数据（一般为逻辑删除，更新del_flag字段为1）
	 * @param entity
	 * @return
	 */
	public int delete(T entity);
	public String getCategorySnMax(T entity);
	public List<T> getAll(String id);

	
}